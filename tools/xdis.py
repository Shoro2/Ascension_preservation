#!/usr/bin/env python3
"""Disassemble a VA range out of Extensions.dll (read-only).  usage: dis.py <va-hex> [nbytes]"""
...
def cstr(va):
    ...
    s = d[o:e]
    return s.decode("latin1") if 0 < len(s) < 200 and all(0x20 <= c <= 0x7e for c in s) else None
...
if __name__ == "__main__":
    dis(int(sys.argv[1], 16), int(sys.argv[2], 0) if len(sys.argv) > 2 else 0x140)
